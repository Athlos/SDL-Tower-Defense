...
void QuadTree::Subdivide()
{
	...
	// Create quad trees around new bounds
...
std::vector<Entity*> QuadTree::QueryPoint(Position* point)
{
	...
	for (int i = 0; i < m_points.size(); ++i) // Chcck all points
	{
...
bool QuadTree::Insert(Entity* pos)
{
	...
	if (m_points.size() >= NODE_CAPACITY && m_topLeft == 0) // Subdivide if you reach max capacity
	{
		Subdivide();
	}
	...
	else // No child trees, insert at this level
	{
		m_points.push_back(pos);
		return true;
	}
...
{
	// Check intersection in all child nodes
